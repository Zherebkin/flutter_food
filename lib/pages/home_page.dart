...
class _HomePageState extends State<HomePage> {
  ...
  Future<List<dynamic>> _getCityCollectionIds() async {
    ...
    headers: {"user-key": YOUR_ZOMATO_API_KEY, "Accept": "application/json"});
    var jsonData = json.decode(reqData.body)['collections'];
    print(reqData.statusCode);
    int len = jsonData.length < 3 ? jsonData.length : 1;
          ...
          return <Widget> [
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                title: Text('Zherebkin Food', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                centerTitle: true,
              ),
            ),
          ];
                    ...
                    if(snapshot.data == null) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return CarouselTemplate(cityId: 51, collectionName: snapshot.data[index + (snapshot.data.length / 2).toInt()], cityCollectionId: snapshot.data[index]);
                        }
                      );
                    }
                  }
